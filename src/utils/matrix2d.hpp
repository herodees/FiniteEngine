...
namespace fin
{
    ...
    public:
        ...
        template <typename T>
        inline T transform_point(const T& pt) const
        {
            return T{ sx * pt.x + shx * pt.y + tx, shy * pt.x + sy * pt.y + ty };
        }

        template <typename T>
        inline T transform_point_inv(const T& pt) const
        {
        ...
        matrix2d& pseudo3DRotate(float angleX, float angleY, float angleZ);
        matrix2d& tilt(float sx, float sy);
    ...
    inline matrix2d& matrix2d::pseudo3DRotate(float angleX, float angleY, float angleZ)
    {
        // Convert angles from degrees to radians
        float radX = angleX * float(M_PI) / 180.f;
        float radY = angleY * float(M_PI) / 180.f;
        float radZ = angleZ * float(M_PI) / 180.f;

        // Skew the matrix to simulate a pseudo-3D rotation effect around the x and y axes
        // This is done by applying a skew based on the angle
        matrix2d skewMatrixX(1.0f, tanf(radX), 0.0f, 1.0f, 0.0f, 0.0f);
        matrix2d skewMatrixY(1.0f, 0.0f, tanf(radY), 1.0f, 0.0f, 0.0f);

        // Combine the skew transformations
        premultiply(skewMatrixX);
        premultiply(skewMatrixY);

        // Now apply a standard 2D rotation around the Z-axis
        float cosZ = cosf(radZ);
        float sinZ = sinf(radZ);

        matrix2d rotationZ(cosZ, -sinZ, sinZ, cosZ, 0.0f, 0.0f);
    ...
    inline matrix2d& matrix2d::tilt(float sx, float sy)
    {
        premultiply(matrix2d(1.0f, tanf(sy * float(M_PI) / 180.f), tanf(sx * float(M_PI) / 180.f), 1.0f, 0.0, 0.0));
        return *this;
    }
