...
namespace fin
{
    ...
    bool EndDefaultMenu(ImGui::CanvasParams& canvas)
    {
        ImGui::Line()
            .Spring()
            .PushStyle(ImStyle_Button, -98, false)
            .Tooltip("Reset zoom to 100%")
            .Format(ICON_FA_MAGNIFYING_GLASS "%.0f%%", canvas.zoom * 100.f)
            .PopStyle()
            .Space()
            .Space()
