...
namespace fin
{
    ...
    public:
        ...
        void ShowHeader(const char* label, const char* desc)
        {
            ImGui::NewLine();
        ...
        void OnUpdateTags()
        {
            ShowHeader("Tags Settings",
                       "Define special tags for layers and objects");
                                  ...
                                  ImGuiWindowFlags_NoBackground))
            {
                for (int32_t n = 0; n < 32; n++)
                {
                    ...
                    ImGui::Text("Tag %d", n);
                    ImGui::SameLine(60);
                    ImGui::InputText("##tag", &_scene.GetTag(n), ImGuiInputTextFlags_EnterReturnsTrue);
        ...
        bool OnUpdate() override
        {
            ...
            if (ImGui::BeginChild("left pane", ImVec2(140, 0), ImGuiChildFlags_FrameStyle | ImGuiChildFlags_ResizeX, 0))
            {
                ...
                if (ImGui::Selectable(" " ICON_FA_ICONS " Tags", selected == PropertyTags))
    ...
    std::string& Scene::GetTag(int32_t index)
    {
        return _tags[index];
    }

    int32_t Scene::GetTag(std::string_view tag) const
    {
        for (auto& t : _tags)
        {
            if (t == tag)
    ...
    std::string Scene::GetTagName(uint32_t tag) const
    {
        std::string str;
        for (auto& t : _tags)
        {
    ...
    void Scene::Serialize(msg::Var& ar)
    {
        ...
        msg::Var tags;
        for (int32_t i = 0; i < _tags.size(); i++)
        {
            tags.push_back(_tags[i]);
        }
        ar.set_item("tags", tags);
    ...
    bool Scene::ImguiTagInput(const char* label, uint32_t& tag) const
    {
        bool ret = false;
        if (ImGui::BeginCombo(label, GetTagName(tag).c_str()))
        {
            for (int32_t i = 0; i < _tags.size(); i++)
            {
                if (_tags[i].empty())
                    continue;
                ret = ImGui::CheckboxFlags(_tags[i].c_str(), &tag, 1 << i);
            }
            ImGui::EndCombo();
        }
        return ret;
